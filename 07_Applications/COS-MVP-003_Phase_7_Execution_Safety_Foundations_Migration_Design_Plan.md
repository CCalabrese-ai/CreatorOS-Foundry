...
**Alignment note (v1.1):** this document was originally written while the compensation-evidence storage question (Decision Record Decision 3) remained explicitly open, pending the Shared Approval/Evidence Primitive's ratification. That question has since been resolved — dedicated `compensation_evidence` storage, Approved with Conditions, per `COS-MVP-003_Phase_7_Decision_Ratification_Record.md` and `COS-MVP-003_Phase_7_Decision_Ratification_Tracker.md`. This revision updates every location that assumed the storage question was still open. **It does not add `compensation_evidence` to this migration's own sequence, reopen or re-decide D08, alter any other Phase 7 decision, or change any ratification record — it brings this document's language into alignment with a decision already made elsewhere. Sequencing `compensation_evidence` into an actual migration remains separate, future planning work.**
...
**Explicitly excluded:**
- **The `compensation_evidence` entity.** Its storage *location* is now resolved — a dedicated table, per the D08 ratification (`COS-MVP-003_Phase_7_Decision_Ratification_Record.md`, `COS-MVP-003_Phase_7_Decision_Ratification_Tracker.md`), not reuse of the Shared Approval/Evidence Primitive's tables. Its migration *sequencing* is not resolved — this plan's own scope was bounded to the four entities whose design was complete when it was written, and `compensation_evidence` was not among them at that time. A future revision of this plan, or a follow-on migration plan, would need to sequence its introduction, subject to the two conditions the D08 resolution attached: (1) existing governance/security conventions — revoke-first grants, three-tier RLS, `SECURITY DEFINER` implementation in `creator_os_private`, a thin `public` wrapper, no direct application-role writes; (2) a future migration or consolidation onto the Shared Approval/Evidence Primitive remains possible if that primitive is later extended with the fields compensation evidence requires. Compensation *authority logic* (the role-overlay check) may still be scoped for a future migration referencing this now-settled storage location — no compensation table is created by this migration as currently scoped.
...
- **The `compensation_evidence` entity is deliberately not sequenced at all** in this plan, consistent with Section 2's exclusion. Its storage location is now settled (a dedicated table), but a future migration's own sequencing would still need to determine where it fits relative to `recovery_actions` — not something this plan pre-empts.
...
- **Shared Approval/Evidence Primitive.** No migration-level dependency — this migration does not require the primitive's own migration to have occurred first, per Decision Record Decision 6 and the Schema Design Review's Section 8. The primitive's decisions (D01–D05) have since been ratified; no rework of this migration was required as a result, since nothing here references its tables. The compensation-storage decision (D08) has been resolved in favor of a dedicated table, not primitive reuse — that dedicated table's own future migration remains separate, future work, not a revision of this one.
...
Consolidated from every prior Execution Safety Foundations document, not newly introduced here. One item previously listed — compensation-evidence storage — has since been resolved and is noted below for traceability, not silently dropped:

- **Resolved since this plan was first written: the compensation-evidence storage question (Decision Record Decision 3).** Previously excluded from this migration's scope pending the Shared Approval/Evidence Primitive's ratification. Now resolved — dedicated `compensation_evidence` storage, Approved with Conditions, per `COS-MVP-003_Phase_7_Decision_Ratification_Record.md`. This resolves the storage *location* question; the entity remains excluded from this specific migration's own sequence (Section 2), which is a separate, still-open sequencing question, not a decision question.
- All seven Execution Safety Foundations Decision Record decisions have been ratified (D01–D07 chain notwithstanding naming — see the Phase 7 Decision Ratification Tracker for exact per-decision status, including D07's two attached conditions).
...
- Whether `authority_role_at_decision` should be captured on `recovery_actions` (Schema Design Review Section 11) — `compensation_evidence` already carries this field per its resolved design; whether `recovery_actions` needs it too remains open.
...
| 1.1 | Documentation alignment pass reflecting the resolved D08 compensation-evidence storage decision (dedicated table, Approved with Conditions): updated Section 2's exclusion rationale, Section 3's additive-strategy note, Section 5's entity-introduction note, Section 10's primitive-relationship note, and Section 11's open-decisions list to distinguish the now-resolved storage *location* question from the still-open migration *sequencing* question. `compensation_evidence` remains excluded from this migration's own sequence — this alignment resolves what it is called and where it lives, not when it is built. No other Phase 7 decision, ratification record, or D01–D19 outcome was altered. No SQL written, no migration created, no implementation performed, no release status changed. |
