...
**Design status: provisional, pending the deferred storage decision.** Decision Record Decision 3 explicitly declined to choose between a dedicated compensation-evidence table and reuse of the Shared Approval/Evidence Primitive's own tables once ratified. The field list below is presented **only for the case that a dedicated entity is eventually chosen** — if the Shared Approval/Evidence Primitive is chosen instead, this entity would not be created, and compensation authorization would instead populate that primitive's `approval_decision_evidence`-equivalent table with a `compensation` action type or subject reference. Neither this document nor any prior one selects between these.

**Required domain fields, if a dedicated entity is chosen:** `compensation_record_id`, `workspace_id`, `run_id`, `actor_user_id`, `precondition_verified` (boolean, recording that the stated precondition was checked, per `Workflow_Design_Standards.md`), `is_irreversible_limit` (boolean, per the same source's "irreversible limits" category), `authority_role_at_decision`, `system_owner_signoff_id` (nullable — populated only when `is_irreversible_limit` is true, per Decision Record Decision 3's authority conclusion), `is_emergency`, `decided_at`, `reason`.

**Key relationships (if chosen):** references the `run` being compensated.
...
- `runs`/`run_steps` (already specified in `Schema_Specification.md`, generalized further by the Technical Design's Section 3) are the anchor every other entity in this document references — incidents, recovery actions, and compensation records all trace back to a specific run.
...
- **Recovery actions and compensation records are not themselves stateful entities** — each row is a point-in-time decision record (approved/denied, or executed), not a multi-state lifecycle of its own, distinguishing them from `incidents` and `runs`. This mirrors how `document_approval_evidence` records a decision without itself having a lifecycle separate from the document it concerns.
...
- **No application role receives `UPDATE` or `DELETE`** on execution audit events, recovery actions, or (if chosen) compensation records — the only mutation path is the governed function chain named in the Implementation Specification's Section 5, and even that path only ever inserts new rows.
...
- **Shared Approval/Evidence Primitive.** Bounded exactly as Decision Record Decision 6 established: no schema-level dependency for the four fully-specified entities in Section 3 (incident model, execution audit events, checkpoints/idempotency records, recovery actions). The one entity left provisional — compensation records — is provisional specifically because its eventual home may be this schema or the primitive's own tables, a decision this document does not make. No entity proposed here registers into `governed_subjects`.
...
- Confirmation that the proposed entity names (`incidents`, `execution_lifecycle_events` or equivalent, `recovery_actions`, and — if chosen — a compensation entity) do not collide with any existing or separately-planned object, including the already-specified `incidents`/`runs`/`run_steps` entries in `Schema_Specification.md` itself, which this document generalizes rather than duplicates.
...
These are new questions this schema-level detail work surfaced, or explicit carry-forwards from prior documents, none resolved here:
...
- **Compensation records: dedicated entity vs. Shared Approval/Evidence Primitive reuse** — carried forward unresolved from Decision Record Decision 3, gated on that primitive's own ratification per Decision 6.
- **Whether `authority_role_at_decision` should be captured on `recovery_actions` as well as `compensation_records`** — proposed only for `compensation_records` in Section 3 by analogy with the Phase 7.1 Schema Design Review's own open question on this exact field; whether `recovery_actions` needs the same field is not decided here.
