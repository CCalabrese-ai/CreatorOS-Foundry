...
- **Purpose:** generalize the pattern already proven for documents (`document_workflow_evidence`, `document_approval_evidence`, `document_lifecycle_events`) into one reusable capability — a single, governed way to record "who approved what, against which version, with what evidence" — rather than a document-specific feature. This is not a novel concept being introduced here: `03_AI_Workforce/Agent_Execution_Framework.md` already defines an "Approval service" component in its own architecture ("Records exact decision, scope, approver, conditions, and expiry"), independently of the document pattern. This primitive is Creator OS **consolidating** two already-designed approval concepts — the proven document pattern and the specified Agent Execution approval service — into one shared capability, not inventing a third.
- **User value:** indirect but structural — without this, every future governed entity (workflows, agents) would otherwise reinvent its own approval bookkeeping, each with its own subtly different guarantees, including duplicating the Approval service already specified for agents.
- **Technical complexity:** Low. Both source patterns already exist — the document evidence pattern is implemented and live-validated; the Agent Execution Framework's Approval service is specified but not yet built. The main work is reconciling them into one abstraction where `subject_type`/`subject_id` can reference a document, a workflow, or an agent identically, consistent with the `approvals` table already present in `05_Database/Schema_Specification.md`'s Identity and Governance section.
- **Dependencies:** The existing document evidence tables and `transition_document_lifecycle` pattern (exists, proven) and `03_AI_Workforce/Agent_Execution_Framework.md`'s Approval service concept (exists, specified, not yet built).
...
- **Technical complexity:** Low-to-moderate, and lower than originally assessed. **Correction from the prior draft:** this is not greenfield design. `06_Automations/Workflow_Design_Standards.md` already requires "Recovery: Pause, resume, replay, manual intervention, and rollback" for every workflow and defines an explicit Compensation model (preconditions, authority, irreversible limits, evidence) rather than assumed rollback. `06_Automations/Automation_Architecture.md` already names a Control plane component ("Manages deployment, pause, rollback, migration, and retirement") and an Observability layer that captures incidents. `06_Automations/Workflow_Registry.md` already registers **WF-010 Incident Response** (Security Owner, "Containment, evidence, recovery, and review," status Proposed). The `incidents` table is already schema-specified, and `runs`/`run_steps` provide most of what an execution audit trail needs structurally. The real work in Phase 7 is **generalizing and implementing these already-designed patterns as shared Creator OS primitives** — consumable by the Workflow Engine and Agent Execution Layer in Phase 8 — not inventing what "undo" means from nothing, plus adding idempotency/checkpointing for safe replay and resumable execution, which is not yet explicitly covered by the existing `06_Automations` designs and is the one genuinely new piece here.
- **Dependencies:** `06_Automations/Workflow_Design_Standards.md`'s Recovery and Compensation requirements, `06_Automations/Automation_Architecture.md`'s Control plane and Observability layer, and `06_Automations/Workflow_Registry.md`'s WF-010 (all exist, designed, not yet implemented as a shared primitive). Does not depend on the Agent Registry or Tool Registry, which is why it can and should land alongside them rather than after.
- **Recommended phase:** **Phase 7 (Foundation), scheduled explicitly so it exists before Phase 8's Agent Execution Layer begins, not concurrently with it and not after.** This directly addresses the System Charter's requirement that material actions have "a recovery path," and implements what `06_Automations` already designed rather than deferring it further.
...
- Execution Safety Foundations — generalizing the already-designed recovery, compensation, and incident-response patterns from `06_Automations` (Workflow Design Standards, Automation Architecture, WF-010) into shared primitives, plus new idempotency/checkpointing for safe replay and resumable execution, landed here specifically so they exist *before* Phase 8, not alongside or after it
...
- [Workflow Registry](../06_Automations/Workflow_Registry.md) (including WF-010 Incident Response), [COS-WF-001 Document Creation Workflow](../06_Automations/COS-WF-001_Document_Creation_Workflow.md)
- [Workflow Design Standards](../06_Automations/Workflow_Design_Standards.md) — existing Recovery requirements and Compensation model that Execution Safety Foundations generalizes
- [Automation Architecture](../06_Automations/Automation_Architecture.md) — existing Control plane and Observability/incident concepts
...
| 1.2 | Corrected per final integrity review, sequence and phase assignments unchanged: replaced the inaccurate claim that recovery/rollback design "does not yet exist anywhere in this repo's specifications" — it does, in `06_Automations/Workflow_Design_Standards.md` (Recovery requirements, Compensation model), `06_Automations/Automation_Architecture.md` (Control plane, Observability/incident concepts), and `06_Automations/Workflow_Registry.md` (WF-010 Incident Response); reframed Execution Safety Foundations as generalizing and implementing these existing patterns, not designing from nothing; added idempotency/checkpointing (safe replay, resumable execution) as new, genuinely uncovered scope within that primitive; updated Shared Approval/Evidence Primitive to acknowledge `Agent_Execution_Framework.md`'s existing "Approval service" concept and reframed it as consolidating two already-designed patterns rather than generalizing only the document one; added the three newly-cited documents to References. No release status change, no tags/releases, no source or database changes. |
